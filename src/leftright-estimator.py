        ...
        if received_image:
            ...
            if received_data:
                ...
                if data:
                    ...
                    skeleton = yarp.Bottle()
                    skeleton.addList().read((received_data.get(0).asList()).get(itP))
                    pred = create_bottle((self.id_image, (int(ld[itP, 0]), int(ld[itP, 1])), label, prob_mean, skeleton))
                    ...
                    skeleton = yarp.Bottle()
                    skeleton.addList().read((received_data.get(0).asList()).get(itP))
                    pred = create_bottle((self.id_image, (), 'unknown', -1, skeleton))
